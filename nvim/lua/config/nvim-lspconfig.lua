...
local function show_lsp_status()
  ...
  for _, client in ipairs(clients) do
    ...
    table.insert(status_lines, string.format("- %s (id: %d, buffers: %s)",
      client.name, client.id, table.concat(buffers, ", ")))
  end
...
vim.api.nvim_create_user_command("LspCleanup", function()
  ...
  if cleaned > 0 then
    vim.notify(string.format("%d개의 중복 LSP 클라이언트를 정리했습니다.", cleaned), vim.log.levels.INFO)
  else
