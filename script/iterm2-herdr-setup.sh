...
#
# iTerm2가 실행 중이면 거부한다. 실행 중에 plist를 고치면 iTerm2가 종료할 때
# 메모리 상태로 덮어써 변경이 사라질 수 있다.
#
# 사용법:
#   ./script/iterm2-herdr-setup.sh --dry-run   # 무엇이 바뀌는지만 출력
#   ./script/iterm2-herdr-setup.sh             # 실제 적용 (iTerm2 종료 후 직접 실행)
#
# herdr을 iTerm2 안에서 쓰면 herdr 서버가 iTerm2의 자식 프로세스라 iTerm2를
# 종료할 때 herdr 세션도 함께 죽는다. 종료 전에 실행 중인 에이전트를 정리하고,
# iTerm2를 Cmd+Q로 닫은 뒤 다른 터미널에서 이 스크립트를 직접 실행하면 된다.
#
